...
addpath lib
addpath 'datasets\simulated'
%% generate some training data
loading_sigma = 0.1; % making this big will make the loading have more variability
input_noise_sigma = 5; % making this big will introduce more noise to everything in X.

% we do see separation between the clusters
beta_sigma = 10;
output_noise_sigma = 1;


subplot(211);simulate_multi_mode_with_infrequent_nodes;

loading_sigma = 0.1; % making this big will make the loading have more variability
input_noise_sigma = 10; % making this big will introduce more noise to everything in X.
...
subplot(212);simulate_multi_mode_with_infrequent_nodes;
%% Setup the training dataset.
% training_data.inputs = scores(:,1:2);
% training_data.outputs = y;
training = 1:5000;
testing = 5001:size(simulated_data_set.outputs,1);

% training_data = structfun(@(x) x(training,:),simulated_data_set,'UniformOutput',false);
% testing_data = structfun(@(x) x(testing,:),simulated_data_set,'UniformOutput',false);


training_data = struct();
training_data.model_inputs = simulated_data_set.model_inputs(training,:);
training_data.gsom_inputs = simulated_data_set.gsom_inputs(training,1:2);
training_data.outputs = simulated_data_set.outputs(training,:);
training_data.true_mode = simulated_data_set.true_mode(training,:);
training_data.true_betas = simulated_data_set.true_betas;


testing_data = struct();
testing_data.model_inputs = simulated_data_set.model_inputs(testing,:);
...
gsom.model_parameters.num_components = 3;
