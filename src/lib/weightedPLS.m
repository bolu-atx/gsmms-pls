function [P,Q,T,U,R, ...
                    beta,pctVar,mse,stats] = weightedPLS(X,Y,ncomp,varargin)
...
names = {'cv'                  'mcreps'            'options'    'sample_weights'};
dflts = {'resubstitution'        1                      []   ones(n,1)};
[cvp,mcreps,ParOptions,sample_weights] = internal.stats.parseArgs(names, dflts, varargin{:});
...
% transpose the sample weights if they are not in column vector form.
if (size(sample_weights,1)<size(sample_weights,2))
    sample_weights = sample_weights';
end

if (size(sample_weights,1) ~= n)
    error('Sample weight vector dimension mismatch.');
end

% Center both predictors and response, and do PLS
% apply weighing when centering

% Disabled mean centering, since we'll do this before running the code

% Calculate sample-weighted mean for X and Y;
% meanX = sum(X.*repmat(sample_weights,1,dx),1)./repmat(sum(sample_weights),1,dx);
% meanY = sum(Y.*repmat(sample_weights,1,size(Y,2)),1)./repmat(sum(sample_weights),1,size(Y,2));
...
if nargout <= 2
    [P,Q] = simpls(X0,Y0,ncomp,sample_weights);
    
elseif nargout <= 4
    [P,Q,T,U] = simpls(X0,Y0,ncomp,sample_weights);
    
else
    % Compute the regression coefs, including intercept(s)
    [P,Q,T,U,R] = simpls(X0,Y0,ncomp,sample_weights);
    beta = R*Q';
    beta = [meanY - meanX*beta; beta];
    
    % Compute the percent of variance explained for X and Y
    if nargout > 6
        pctVar = [sum(abs(P).^2,1) ./ sum(sum(abs(X0).^2,1));
                  sum(abs(Q).^2,1) ./ sum(sum(abs(Y0).^2,1))];
    end
    ...
    if nargout > 7
        if isequal(cvp,'resubstitution')
            ...
            for i = 1:ncomp
                X0reconstructed = T(:,1:i) * P(:,1:i)';
                Y0reconstructed = T(:,1:i) * Q(:,1:i)';
    ...
    if nargout > 8
        ...
        stats.T2 = sum( bsxfun(@rdivide, abs(T).^2, var(T,[],1)) , 2);
...
function [P,Q,T,U,R] = simpls(X0,Y0,ncomp,sample_weights)
...
P = zeros(dx,ncomp,outClass);
Q = zeros(dy,ncomp,outClass);
if nargout > 2
    T = zeros(n,ncomp,outClass);
    U = zeros(n,ncomp,outClass);
    if nargout > 4
        R = zeros(dx,ncomp,outClass);
    end
end

% An orthonormal basis for the span of the X loadings, to make the successive
% deflation X0'*Y0 simple - each new basis vector can be removed from S
% separately.
V = zeros(dx,ncomp);

% Modified for sample weighted PLS
%S = X0'*Y0;

% Modified Aug 27th: if the sample weights are too sparse (less than 5%)
% Then proceed with default PLS and not weighted PLS.
if sum(sample_weights) < 0.05*numel(sample_weights)
    warning('Might encounter NaN in SVD due to sparse weights, falling back to non-weighted PLS.');
    sample_weights = ones(n,1);
end
Omega = diag(sample_weights); % diagonalized weighting matrix
S = X0'*Omega*Y0;
...
for i = 1:ncomp
    ...
    % weighted PLS need to change how the norm is done for scores
    normti = sqrt(ti'*(ti.*sample_weights));
    %normti = sqrt(ti'*Omega*ti);
    
    ti = ti ./ normti; % ti'*ti == 1
    
    P(:,i) = X0'*(ti.*sample_weights); % the loadings is also affected by the weights
    %P(:,i) = X0'*Omega*ti; % the loadings is also affected by the weights
    ...
    if nargout > 2
        T(:,i) = ti;
        U(:,i) = Y0*qi; % = Y0*(Y0'*ti), and proportional to Y0*ci
        if nargout > 4
            R(:,i) = ri ./ normti; % rescaled to make ri'*X0'*X0*ri == ti'*ti == 1
        end
    end
    ...
    % Deflate S, i.e. project onto the ortho-complement of the X loadings.
...
if nargout > 2
    % By convention, orthogonalize the Y scores w.r.t. the preceding T,
...
[P,Q,~,~,R] = simpls(X0train,Y0train,ncomp);
XscoresTest = X0test * R;
...
for i = 1:ncomp
    X0reconstructed = XscoresTest(:,1:i) * P(:,1:i)';
    sumsqerr(1,i+1) = sum(sum(abs(X0test - X0reconstructed).^2, 2));

    Y0reconstructed = XscoresTest(:,1:i) * Q(:,1:i)';
