...
function [obj, debug] = gspls_train(gsom, training_data, parameters,debugmode)
if nargin < 4;debugmode = 0;end
...
while(gsom.rms >= ErrTol) && (gsom.netsize < max_netSize) ...
    ...
    for pass = 1:passMax
        ...
        error_contribution = gsom.rmsReg/median(gsom.rmsReg);
        
        for i = 1 : gsom.netsize
            ind = find( bmus(:, 1) == i); % find all the training data belonging to this node
            ...
            if isempty(ind)
                ...
                % as the weighted average location of the neighbor
                % centroids, weighted by the squared modeling error (rmsReg) of
                % each local node
            ...
            for j = 1:gsom.netsize
                %mass ratio is a weighted ratio of the sample size of the current training
                %node against the total number of samples in i and j
                mass_ratio = numel(ind)/(numel(ind)+numel(find( bmus(:, 1) == j)));
                %%h = exp(-Distance(i, j)^2 / 2/(coop)^2 );
                h = exp(-Distance(i, j)^2 / 2/(0.0001)^2 );
                
                codebook_gradient = ( mean(training_data.gsom_inputs(ind, :)) - gsom.codebook(j, :));
                
                %codebook_gradient = codebook_gradient;
                
                % the bigger the prediction error of the data cluster at i,
                % the bigger the attraction
                learning_rate = error_contribution(i)  * h * mass_ratio;
                % dump debug message
                if debugmode == 1
                    display(sprintf('%d    %d-->%d    %0.2f    %0.2f    %0.2f    (%0.2f,%0.2f)',...
                        pass,j,i,mass_ratio,error_contribution(i),h,codebook_gradient(1),...
                        codebook_gradient(2)));
                end
